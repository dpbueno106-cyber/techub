...
function placeNTO(existingSlots, weeks, locations, config) {
    ...
    while (targetDate.getFullYear() <= scheduleYear) {
        if (targetDate.getFullYear() === scheduleYear) {
            const startIndex = sortedWeeks.findIndex(w => new Date(w.startDate).getTime() >=
                targetDate.getTime());
            if (startIndex < 0) {
                console.warn(`placeNTO: No available week on/after ${targetDate.toISOString().slice(0, 10)} for occurrence ${occurrence + 1}.`);
            }
            else {
                const block = sortedWeeks.slice(startIndex, startIndex + blockLength);
                ...
                const alreadyUsed = block.some(w => usedWeeks.has(w.weekNumber));
                if (!isConsecutive) {
                    console.warn(`placeNTO: Could not find ${blockLength} consecutive unblocked weeks starting near ${targetDate.toISOString().slice(0, 10)} (occurrence ${occurrence + 1}).`);
                }
                else if (alreadyUsed) {
                    console.warn(`placeNTO: Weeks starting ${block[0].startDate} are already reserved; skipping occurrence ${occurrence + 1}.`);
                }
                else {
