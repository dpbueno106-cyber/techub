...
function placeNTO(existingSlots, weeks, locations, config) {
    ...
    const sortedWeeks = [...weeks]
        .filter(w => !w.blocked)
        .sort((a, b) => parseLocalDate(a.startDate).getTime() -
    ...
    const scheduleYear = parseLocalDate(sortedWeeks[0].startDate).getFullYear();
    ...
    while (targetDate.getFullYear() <= scheduleYear) {
        if (targetDate.getFullYear() === scheduleYear) {
            ...
            const searchStartIndex = sortedWeeks.findIndex(w => parseLocalDate(w.endDate).getTime() >=
                targetDate.getTime());
            let placed = false;
            let sawConsecutiveOption = false;
            if (searchStartIndex >= 0) {
                for (let idx = searchStartIndex; idx < sortedWeeks.length; idx++) {
                    const candidateStart = parseLocalDate(sortedWeeks[idx].startDate).getTime();
                    ...
                    const isConsecutive = block.length === blockLength &&
                        block.every((w, i) => {
                            if (i === 0)
                                return true;
                            const diffDays = (parseLocalDate(w.startDate).getTime() -
            ...
            if (!placed) {
                if (searchStartIndex < 0) {
                    console.warn(`placeNTO: No available (non-holiday) week on/after ${formatLocalDate(targetDate)} for occurrence ${occurrence + 1}.`);
                }
                else if (!sawConsecutiveOption) {
                    console.warn(`placeNTO: Could not find ${blockLength} consecutive non-holiday weeks between ${formatLocalDate(targetDate)} and ${formatLocalDate(windowEnd)} (occurrence ${occurrence + 1}).`);
                }
                else {
                    console.warn(`placeNTO: Every candidate week between ${formatLocalDate(targetDate)} and ${formatLocalDate(windowEnd)} overlaps a previous NTO occurrence; skipping occurrence ${occurrence + 1}.`);
                }
            }
        }
        occurrence++;
